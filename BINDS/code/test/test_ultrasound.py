...
root = '/public/home/liyh2022/projects/BINDS/evaluation/ultrasound/'
data = np.load(os.path.join(root, 'ultrasound_predictions.npz'), allow_pickle=True)

preds_risk = data['preds_risk'].item()['ultrasound']
preds_subtype = data['preds_subtype'].item()['ultrasound']
...
test_cancer_risk(
    ...
    title='Ultrasound_Risk'
)

tsne_plot(
    features=features_risk,
    gt=gts_risk,
    title='Ultrasound Risk t-SNE',
...
test_cancer_subtype(
    ...
    title='Ultrasound_Subtype'
)

tsne_plot(
    features=features_subtype,
    gt=gts_subtype,
    title='Ultrasound Subtype t-SNE',
