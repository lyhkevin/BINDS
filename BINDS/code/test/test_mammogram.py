...
root = '/public/home/liyh2022/projects/BINDS/evaluation/mammogram/'
data = np.load(os.path.join(root, 'mammogram_predictions.npz'), allow_pickle=True)

preds_risk = data['preds_risk'].item()['mammogram']
preds_subtype = data['preds_subtype'].item()['mammogram']
...
test_cancer_risk(
    ...
    title='Mammogram_Risk'
)

tsne_plot(
    features=features_risk,
    gt=gts_risk,
    title='Mammogram Risk t-SNE',
...
test_cancer_subtype(
    ...
    title='Mammogram_Subtype'
)

tsne_plot(
    features=features_subtype,
    gt=gts_subtype,
    title='Mammogram Subtype t-SNE',
