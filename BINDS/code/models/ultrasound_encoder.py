...
class ultrasound_encoder(nn.Module):
    def __init__(self, depth_encoder, depth_fusion, backbone='ResNet', num_classes=[2], img_size=224, patch_size=16,
                 embed_dim=384, embed_dim_pathology=768, multi_modal=False, alignment=False, dropout=0.1):
        super(ultrasound_encoder, self).__init__()
        ...
        print('Ultrasound backbone:', backbone)
        if backbone == 'ResNet':
        ...
        self.tokens = nn.ParameterList([nn.Parameter(torch.zeros(1, 1, embed_dim)) for _ in num_classes])
        self.embedding_1 = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.embedding_2 = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.pos_embedding = nn.Parameter(torch.zeros(1, self.patch_num, embed_dim))
    ...
    def forward_with_grad(self, x):
        ...
        # print(f"第一个位置更大: {count_first} 个, 第二个位置更大: {count_second} 个")
