...
if __name__ == '__main__':
    ...
    opt.backbone_mammogram = 'ResNet'
    opt.oversample = True
    opt.augmentation = True
    opt.oversample_targets = ['Risk', 'Subtype']
    opt.save_snapshot = True
    opt.snapshot_interval = 1000
    opt.acc_threshold = [0.99, 0.99]

    opt.accumulation_steps = 1
    opt.loss_weight = [None, [1.0, 20.0]]
    opt.oversample_rates = [[1, 0], [0, 3]] 

    opt.key_modalities = ['mammogram']
    opt.weight_save_path = '../weight/mammogram/'
    opt.img_save_path = '../snapshot/mammogram/'
    ...
    if opt.alignment:
        opt.modalities = ['mammogram', 'pathology']
    ...
    batch_size = opt.batch_size_alignment if opt.alignment else opt.batch_size
    train_dataset, train_loader = get_dataloader(
        batch_size=batch_size, shuffle=opt.shuffle,
    ...
    encoder = mammogram_encoder(
        embed_dim=opt.embed_dim, backbone=opt.backbone_mammogram,
        num_classes=num_classes, depth_encoder=opt.depth_encoder_mammogram,
        depth_fusion=opt.depth_fusion_mammogram,
    ...
    for epoch in range(0, opt.epoch):
        ...
        for i, samples in enumerate(tqdm(train_loader)):
            ...
            if opt.alignment:
                ...
                for sample in samples:
                    imgs.append(sample['mammogram'])
                    has_img.append(sample['has_mammogram'])
        ...
        if (epoch + 1) % opt.validation_interval == 0:
            ...
            if avg_auc > best_avg_auc:
                ...
                best_epoch = epoch + 1
                torch.save(encoder.state_dict(), opt.weight_save_path + 'best_encoder_mammogram.pth')
        ...
        if (epoch + 1) % opt.weight_save_interval == 0:
            torch.save(encoder.state_dict(), opt.weight_save_path + 'encoder_mammogram_' + str(epoch + 1) + '.pth')
    ...
    torch.save(encoder.state_dict(), opt.weight_save_path + 'encoder_mammogram.pth')
