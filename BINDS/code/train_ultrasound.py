...
if __name__ == '__main__':
    ...
    opt.backbone_ultrasound = 'ResNet'
    opt.oversample = True
    ...
    opt.oversample_rates = [[0, 0], [0, 1]]

    opt.key_modalities = ['ultrasound']
    opt.weight_save_path = '../weight/ultrasound/'
    opt.img_save_path = '../snapshot/ultrasound/'
    ...
    if opt.alignment:
        opt.modalities = ['ultrasound', 'pathology']
    ...
    encoder = ultrasound_encoder(
        embed_dim=opt.embed_dim, backbone=opt.backbone_ultrasound,
        num_classes=num_classes, depth_encoder=opt.depth_encoder_ultrasound,
        depth_fusion=opt.depth_fusion_ultrasound,
    ...
    for epoch in range(0, opt.epoch):
        ...
        for i, samples in enumerate(tqdm(train_loader)):
            ...
            if opt.alignment:
                ...
                for sample in samples:
                    imgs.append(sample['ultrasound'])
                    has_img.append(sample['has_ultrasound'])
        ...
        if (epoch + 1) % opt.validation_interval == 0:
            ...
            if avg_auc > best_avg_auc:
                ...
                best_epoch = epoch + 1
                torch.save(encoder.state_dict(), opt.weight_save_path + 'best_encoder_ultrasound.pth')
        ...
        if (epoch + 1) % opt.weight_save_interval == 0:
            torch.save(encoder.state_dict(), opt.weight_save_path + 'encoder_ultrasound_' + str(epoch + 1) + '.pth')
    ...
    torch.save(encoder.state_dict(), opt.weight_save_path + 'encoder_ultrasound.pth')
